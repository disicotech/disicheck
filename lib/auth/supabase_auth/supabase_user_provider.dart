...
class DisicheckSupabaseUser extends BaseAuthUser {
  DisicheckSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> disicheckSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DisicheckSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
